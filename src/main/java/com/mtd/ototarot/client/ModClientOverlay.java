...
@EventBusSubscriber(modid = OtOtArot.MOD_ID, bus = EventBusSubscriber.Bus.GAME, value = Dist.CLIENT)
public class ModClientOverlay {
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        if (headStack.getItem() instanceof PerrotMaskItem) {
            renderTextureOverlay(event.getGuiGraphics(), PERROT_OVERLAY_TEXTURE, 1.0F);
        }
    }
    ...
    private static void renderTextureOverlay(GuiGraphics guiGraphics, ResourceLocation shaderLocation, float alpha) {
        ...
        guiGraphics.blit(shaderLocation, 0, 0, -90, 0.0F, 0.0F, width, height, width, height);
